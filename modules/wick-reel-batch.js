...
const REEL_LANES = ["CREDIT", "INVESTING"];
...
// Retries for the same reason the carousel uploader does: the cover and thumb
// are already generated by this point, so a transient "fetch failed" would
// discard paid work. Seen live 2026-08-09 on the carousel side.
async function upload(buf, key, attempts = 4) {
  let lastErr;
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      if (i < attempts) {
        ...
        console.warn(`[WickReels] upload attempt ${i}/${attempts} failed (${err.message}), retrying in ${backoff / 1000}s`);
        await new Promise((r) => setTimeout(r, backoff));
      }
    }
  }
  throw new Error(`upload ${key} failed after ${attempts} attempts: ${lastErr?.message}`);
}
